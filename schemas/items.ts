...
const starts = z.string("repeat.starts must be a string").trim().refine(value => Validation.dateTime(value), "repeat.starts must be a valid ISO 8601 datetime")
const ends = z.string("repeats.ends must be a string").trim().refine(value => Validation.dateTime(value), "repeats.ends must be a valid ISO 8601 datetime").optional()
