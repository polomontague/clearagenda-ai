...
import { OnceStep, RepeatingStep } from "@/types/Task"
...
export const POST = async (req: NextRequest, props: { params: Promise<{ task_id: string, step_id: string }> }) => {
    try {
        ...
        if (task.occurs == "once") {
            let completed = (step as OnceStep).completed
            if (!completed) completed = await TasksDAO.updateStepCompleted(params.step_id, new Date().toISOString())
            return Response.ok({ completed })
        }
        if (task.occurs === "repeating" && body.date) {
            let completion = (step as RepeatingStep).completions.find(completion => completion.date === body.date)
            if (!completion) completion = await TasksDAO.createStepCompletion(params.step_id, {
                date: body.date,
